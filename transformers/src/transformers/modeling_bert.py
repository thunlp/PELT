...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        extended_attention_mask = extended_attention_mask.to(embedding_output.dtype) #(dtype=next(self.parameters()).dtype)  # fp16 compatibility
...
class BertLMPredictionHeadNew(nn.Module):
    ...
    def forward(self, hidden_states, masked_token_indexes=None):
...
class BertOnlyMLMHeadNew(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertLMPredictionHeadNew(config)

    def forward(self, sequence_output, masked_token_indexes=None):
        prediction_scores = self.predictions(sequence_output, masked_token_indexes=masked_token_indexes)
        return prediction_scores


class BertSimplekeplerForPretraining(BertPreTrainedModel):

    def __init__(self, config):
        super().__init__(config)

        self.bert = BertModel(config)
        self.cls = BertOnlyMLMHeadNew(config)
    ...
    def forward(
        ...
        bsz = input_ids.shape[0]
        input_ids_plus = torch.cat([input_ids, h_ids, t_ids, cooccur_ids], dim=0)
        input_mask_plus = torch.cat([attention_mask, h_mask, t_mask, cooccur_mask], dim=0)

        
        outputs_plus = self.bert(
            input_ids_plus,
            attention_mask=input_mask_plus,
        )
        embed_plus = outputs_plus[1]
        
        h = embed_plus[bsz: bsz*2]
        t = embed_plus[bsz*2: bsz*3]
        r = embed_plus[bsz*3: ]

        sequence_output = outputs_plus[0][:bsz]
        ...
        # (bsz, K, dim)
        if neg_entity_ids is not None:
            ke_entity_ids = torch.cat([neg_entity_ids, h_entity_ids.view(-1), t_entity_ids.view(-1)])
            neg_entity = torch.cat([neg_entity_embed, h, t])
        else:
            ke_entity_ids = torch.cat([h_entity_ids.view(-1), t_entity_ids.view(-1)])
            neg_entity = torch.cat([h, t])
        ...
        return (loss, masked_lm_loss, ke_loss, h, t)
...
class BertSimpleLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = BertPredictionHeadTransform(config)
...
class BertSimpleOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertSimpleLMPredictionHead(config)

    def forward(self, sequence_output):
        hidden_states = self.predictions(sequence_output)
        return hidden_states




class BertForRep(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = BertModel(config)
        self.cls = BertSimpleOnlyMLMHead(config)
...
class BertForHeadTailSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        bsz = sequence_output.shape[0]
        h_rep = sequence_output[torch.arange(bsz), h_position]
        t_rep = sequence_output[torch.arange(bsz), t_position]
