...
class Transformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, copy_rate=None, tokens_prob=None):
        ...
        # a = time.time()
        # aa = 0
        final_hidden_states = None#torch.zeros_like(hidden_states)
        mini = 16 
            ...
            if (i in [1, mid]) and (num_items > mini):
                ...
                if copy_rate is not None:
                    if copy_rate.sum()>0:
                        with torch.no_grad():
                            if tot_zoom is None:
                                guide = tokens_prob[:,i-1,:]   # (bsz, 8, seq_len)
                            else:
                                guide = torch.matmul(tot_zoom.transpose(1,2), tokens_prob[:, i-1, :].unsqueeze(-1)).squeeze(-1)
                        ...
                        caiyang_prob = prob * (1-copy_mask) + t_p * copy_mask
                    else:
                ...
                with torch.no_grad():
                    # flat_top_indices = self.flatten_and_batch_shift_indices(top_indices, num_items)

                    del_mask = (1 - selected_token) * seq_mask #self.get_ones(bsz*num_items, device)
                    # del_mask[flat_top_indices] = 0
                    # del_mask = del_mask.view(bsz, num_items)
        ...
        outputs = (final_hidden_states, tot_select_loss, Ls)
        # hidden_state = x
        # for i, layer_module in enumerate(self.layer):
        #     if self.output_hidden_states:
...
class AUTOQADistilBertModel(DistilBertPreTrainedModel):
    def __init__(self, config):
    ...
    Examples::
        ...
        tfmr_output = self.transformer(x=inputs_embeds, attn_mask=attention_mask, head_mask=head_mask,
                    copy_rate=copy_rate, tokens_prob=tokens_prob)
...
class AUTOQADistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.distilbert = AUTOQADistilBertModel(config)
    ...
    Examples::
        ...
        outputs = (start_logits, end_logits,) + distilbert_output[2:]
