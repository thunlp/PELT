                    ...
                    if copy_rate.sum()>0:
                        ...
                        with torch.no_grad():
                            if tot_zoom is None:
                                ...
                                guide = torch.matmul(tot_zoom, tokens_prob[:, i-1, :].unsqueeze(-1)).squeeze(-1)
...
class AUTODistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    @add_start_docstrings_to_callable(DISTILBERT_INPUTS_DOCSTRING)
    def forward(self, input_ids=None, attention_mask=None, head_mask=None, inputs_embeds=None, labels=None, copy_rate=None, tokens_prob=None):
        r"""
